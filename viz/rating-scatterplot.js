...
let currentMode = "popularity"; // "popularity" | "engagement"
...
async function ratingScatter() {
  ...
  // SVG SETUP
  // --------------------------------------------------
  const { width, height, margin } = scatterConfig;
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;

  const svg = d3
  .select("#rating-scatter")
  .append("svg")
...
// --------------------------------------------------
  ...
  updateScatter(currentMode);
    ...
    `.scatter-toggle .mode-btn[data-mode="${currentMode}"]`
    );
    if (initialActive) {
