...
let currentYearMode = "popularity"; // "popularity" | "engagement"
...
async function yearTrends() {
  ...
  const { width, height, margin } = yearConfig;
  ...
  function updateYear(mode) {
    ...
    allPts
      .on("mousemove", (event, d) => {
        ...
        updateZoom(currentYearMode, d.year);
  ...
  // Initial render
  updateYear(currentYearMode);

  // --------------------------------------------------
  // TOGGLE STYLING + INTERACTION (MATCHES SCATTER STYLE)
// --------------------------------------------------
  const yearButtons = document.querySelectorAll(".year-toggle .year-mode-btn");
  ...
  const initialYearActive = document.querySelector(
    `.year-toggle .year-mode-btn[data-mode="${currentYearMode}"]`
  );
